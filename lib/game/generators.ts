// generators.ts - simulate content generation (events, advisors, social posts)
...
// Generate a satirical in-game event
export function generateEvent(state: GameState): GameEvent {
  ...
  if (Math.random() < 0.5) {
    return {
      ...
      description:
        "Your recent meme campaign hashtag #SealTheSteel has exploded in popularity, gaining mainstream media attention.",
      options: [], // no player choice (narrative event)
      outcome: {
        cloutDelta: 20,
        supportDelta: { ALL: 2 },
        message:
          "The viral hashtag greatly boosted your movement's clout and gave a small bump to support everywhere.",
      },
    };
  }
  // Interactive event
  const event: GameEvent = {
    ...
    description:
      "A university banned a speaker aligned with your movement, sparking nationwide debate.",
    options: [
      {
        ...
        outcome: {
          ...
          riskDelta: 5,
          message:
            "Your bold stance rallies free-speech supporters nationwide, but also draws increased scrutiny.",
        },
      },
      {
        ...
        outcome: {
          ...
          riskDelta: 2,
          message:
            "Your memes turn the incident into a viral joke, boosting clout with minimal fallout.",
        },
      },
      {
        ...
        outcome: {
          riskDelta: -5,
          message:
            "You decide to stay out of it, avoiding controversy and reducing risk, but missing a chance to gain support.",
        },
      },
    ],
  };
