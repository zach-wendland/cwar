...
const RiskMeter: React.FC<RiskMeterProps> = ({ risk, showLabel = true, size = 'md' }) => {
  const getRiskLevel = () => {
    if (risk >= 90) return { level: 'critical', label: 'CRITICAL', icon: Skull };
    if (risk >= 70) return { level: 'high', label: 'HIGH RISK', icon: AlertTriangle };
    if (risk >= 40) return { level: 'medium', label: 'MODERATE', icon: AlertTriangle };
    ...
    <div className="risk-meter-container">
      {showLabel && (
        <div className="risk-meter-header">
          ...
          <span className={`risk-meter-value risk-meter-value--${level}`}>{risk}%</span>
        </div>
      )}
