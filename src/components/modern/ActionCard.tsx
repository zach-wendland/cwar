...
const ActionCard: React.FC<ActionCardProps> = ({
  ...
  return (
    <motion.button
      className={`action-btn ${disabled ? 'action-btn--disabled' : ''} ${isCriticalHit ? 'action-btn--critical' : ''}`}
      onClick={handleClick}
      disabled={disabled}
      whileHover={disabled ? {} : { scale: 1.02, y: -2 }}
      whileTap={disabled ? {} : { scale: 0.98 }}
      animate={isCriticalHit ? { boxShadow: ['0 0 20px rgba(251, 191, 36, 0)', '0 0 40px rgba(251, 191, 36, 0.5)', '0 0 20px rgba(251, 191, 36, 0)'] } : {}}
