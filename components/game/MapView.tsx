...
// Dynamic import for USA Map (client-side only)
const USAMap = dynamic(
  () => import("@mirawision/usa-map-react").then((mod) => mod.USAMap),
  { ssr: false }
);
...
const MapView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    for (const code in state.support) {
      if (
        prevSupport.current[code] !== undefined &&
    ...
    <GlassPanel title="Support Map" icon={Map} glowColor="green">
      <div className="map-container relative">
        ...
        <div className="usa-map-wrapper">
          {isClient && <USAMap customStates={customStates} />}
        </div>
          ...
          {hoveredState && (
            ...
            >
              <div className="tooltip-header">
                ...
                {getTrend(hoveredState) === "up" && (
                  <TrendingUp size={14} className="text-green-400" />
                )}
                {getTrend(hoveredState) === "down" && (
        ...
        <div className="map-stats mt-3">
          <div
            className={`avg-support-badge ${avgSupport >= 80 ? "victory" : ""}`}
          >
