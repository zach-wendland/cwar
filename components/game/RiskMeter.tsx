...
const ZONE_CONFIG: Record<RiskZone, { icon: React.ElementType; message: string; classes: string }> = {
  SAFE: {
    ...
    classes: "text-risk-safe"
  },
  CAUTION: {
    icon: AlertTriangle,
    message: "Costs +10% | Advisors getting nervous",
    classes: "text-risk-caution"
  },
  DANGER: {
    icon: AlertTriangle,
    message: "Costs +25% | High scrutiny",
    classes: "text-risk-danger"
  },
  CRITICAL: {
    icon: Skull,
    message: "Costs +40% | Actions LOCKED",
    classes: "text-risk-critical"
  }
};
...
const RiskMeter: React.FC<RiskMeterProps> = ({
  ...
  return (
    <div className="risk-meter-container">
      ...
      {/* Risk meter bar - single animated fill without duplicate CSS animation */}
      <div
        className={`risk-meter ${sizeClasses[size]} relative overflow-hidden rounded-full bg-slate-800`}
        role="progressbar"
        aria-valuenow={risk}
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label={`Risk level: ${risk}%, ${zoneInfo.label} zone`}
      >
        <motion.div
          className={`risk-meter__fill absolute top-0 left-0 h-full rounded-full ${
            zone === 'SAFE' ? 'bg-risk-safe' :
            zone === 'CAUTION' ? 'bg-risk-caution' :
            zone === 'DANGER' ? 'bg-risk-danger' :
            'bg-risk-critical'
          }`}
          initial={{ width: 0 }}
